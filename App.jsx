...
// 八卦万物类象（射覆取象用）
const XIANG = [null,
  { ren: "老年男性、长者、领导、父辈", shu: 1, se: "大赤、金白", fang: "西北", shen: "头、骨", wu: "马", tai: "刚健，步伐有力" },
  { ren: "少女、年轻女性、爱说笑者、歌者", shu: 2, se: "白色", fang: "西", shen: "口、舌", wu: "羊", tai: "喜悦，有说有笑" },
  { ren: "中年女性、文化人、戴眼镜者", shu: 3, se: "红、紫、花色", fang: "南", shen: "眼、心", wu: "雉、龟", tai: "亮丽显眼，衣着鲜明" },
  { ren: "长男、青壮年男性、性急者", shu: 4, se: "青、绿", fang: "东", shen: "足、肝", wu: "龙、蛇", tai: "动作快，先动脚、跨步大" },
  { ren: "长女、青年女性、温和文气者", shu: 5, se: "青白、碧绿", fang: "东南", shen: "腿股、气息", wu: "鸡", tai: "进退犹豫，举止轻柔" },
  { ren: "中年男性、奔波劳碌者", shu: 6, se: "黑、深蓝", fang: "北", shen: "耳、肾、血", wu: "猪、鱼", tai: "低调匆忙，或有心事" },
  { ren: "少男、小孩、安静的年轻人", shu: 7, se: "黄、土褐", fang: "东北", shen: "手、鼻、背", wu: "狗", tai: "安静止步，动作迟缓" },
  { ren: "老年女性、母辈、或一群人", shu: 8, se: "黄、黑", fang: "西南", shen: "腹", wu: "牛", tai: "柔缓平稳，人多成群" },
];
...
// ── 卦数选号 ──
// 同一卦恒出同一组号：先以本互变卦数与动爻直接定号，不足之数由卦象种子推演补齐
...
function lotto(r) {
  const seed = ((r.shang * 8 + r.xia) * 6 + r.dong) * 4096 + (r.hu.up * 8 + r.hu.low) * 64 + r.bian.up * 8 + r.bian.low;
  const rng = mulberry32(seed);
  // 卦理基础数：上卦、下卦、动爻、本卦合数、互卦合数、变卦合数、诸数总和
  const base = [
    r.shang, r.xia, r.dong,
    r.shang * 8 + r.xia,
    r.hu.up * 8 + r.hu.low,
    r.bian.up * 8 + r.bian.low,
    r.shang + r.xia + r.dong + r.hu.up + r.hu.low,
  ];
  ...
  const vinci = draw(5, 40);          // VinciCasa 5/40
  const euro = draw(5, 50);           // EuroJackpot 主号 5/50
  const superena = draw(6, 90);       // SuperEnalotto 6/90
  const jolly = (() => {              // SuperEnalotto Jolly：与主号不重复
    let j = ((r.shang + r.xia + r.hu.up + r.hu.low + r.bian.up + r.bian.low + r.dong) - 1) % 90 + 1;
    while (superena.includes(j)) j = 1 + Math.floor(rng() * 90);
    return j;
  })();
  const stars = (() => {              // EuroJackpot 幸运号 2/12：取体、用卦数推演
    const s = new Set([((r.ti === T[r.shang] ? r.shang : r.xia) + r.dong - 1) % 12 + 1]);
    s.add(((r.shang + r.xia) - 1) % 12 + 1);
    while (s.size < 2) s.add(1 + Math.floor(rng() * 12));
    return [...s].sort((a, b) => a - b).slice(0, 2);
  })();
  return { vinci, euro, stars, superena, jolly };
}
  ...
  return (
    <div className="root">
      <style>{`
        ...
        .hint2{font-size:12px;color:#8a7d62;margin-bottom:10px;letter-spacing:.04em;}
        .xgrid{display:grid;grid-template-columns:1fr 1fr;gap:10px 14px;}
        .xitem{display:flex;flex-direction:column;gap:2px;}
        .xk{font-size:11px;color:#a3937a;letter-spacing:.2em;}
        .xv{font-size:13.5px;color:#423b30;line-height:1.6;}
        .xfoot{margin-top:14px;padding-top:10px;border-top:1px dotted #d4c49c;font-size:13px;line-height:1.9;color:#5a5141;}
        .xfoot b{color:#b5382a;}
        .lrow{display:flex;align-items:center;gap:12px;margin:10px 0;flex-wrap:wrap;}
        .lname{display:flex;flex-direction:column;min-width:92px;font-family:'Noto Serif SC',serif;font-weight:600;font-size:14px;color:#423b30;}
        .lname i{font-style:normal;font-size:10px;color:#a3937a;letter-spacing:.08em;}
        .balls{display:flex;gap:6px;flex-wrap:wrap;}
        .balls b{width:31px;height:31px;border-radius:50%;background:#332e26;color:#f3ead8;display:flex;align-items:center;justify-content:center;font-size:13.5px;font-weight:600;box-shadow:inset 0 -2px 0 rgba(0,0,0,.25);}
        .balls b.star{background:#b5382a;}
        .balls b.jolly{background:#c9962f;color:#2e2a24;}
      ...
      <div className="wrap">
        ...
        {r && (
          <div className="paper" key={r.src + r.name + Math.random()}>
            ...
            <div className="judge">
              <h3>取 象 · 射 覆</h3>
              <div className="hint2">用卦为所测之人事，以下按「{r.yong.n}（{r.yong.img}）」取象：</div>
              <div className="xgrid">
                <div className="xitem"><span className="xk">人物</span><span className="xv">{XIANG[T.indexOf(r.yong)].ren}</span></div>
                <div className="xitem"><span className="xk">人数·卦数</span><span className="xv">{XIANG[T.indexOf(r.yong)].shu} 人为主，或取上下卦数之和 {XIANG[r.shang].shu + XIANG[r.xia].shu}</span></div>
                <div className="xitem"><span className="xk">衣色</span><span className="xv">{XIANG[T.indexOf(r.yong)].se}</span></div>
                <div className="xitem"><span className="xk">动静神态</span><span className="xv">{XIANG[T.indexOf(r.yong)].tai}</span></div>
                <div className="xitem"><span className="xk">方位</span><span className="xv">{XIANG[T.indexOf(r.yong)].fang}方来</span></div>
                <div className="xitem"><span className="xk">关联物象</span><span className="xv">{XIANG[T.indexOf(r.yong)].shen}、{XIANG[T.indexOf(r.yong)].wu}</span></div>
              </div>
              <div className="xfoot">
                动爻第{["一", "二", "三", "四", "五", "六"][r.dong - 1]}爻为<b>{r.lines[r.dong - 1] === 1 ? "阳" : "阴"}</b>
                {r.lines[r.dong - 1] === 1 ? "——主男、主左、主先动（先迈左脚、先开左门）" : "——主女、主右、主后动（先迈右脚、动作偏缓）"}。
                互卦「{r.hu.name}」之象可参中途变化，变卦「{r.bian.name}」之象验最终应验。
              </div>
            </div>

            <div className="judge">
              <h3>卦 数 · 选 号</h3>
              <div className="hint2">由本卦、互卦、变卦之数与动爻推演而定——同一卦，必出同一组号。</div>
              {(() => { const lot = lotto(r); return (<>
                <div className="lrow">
                  <span className="lname">VinciCasa<i>5 / 40</i></span>
                  <div className="balls">{lot.vinci.map(n => <b key={n}>{n}</b>)}</div>
                </div>
                <div className="lrow">
                  <span className="lname">EuroJackpot<i>5 / 50 + 2 / 12</i></span>
                  <div className="balls">
                    {lot.euro.map(n => <b key={n}>{n}</b>)}
                    {lot.stars.map(n => <b className="star" key={"s" + n}>{n}</b>)}
                  </div>
                </div>
                <div className="lrow">
                  <span className="lname">SuperEnalotto<i>6 / 90 + Jolly</i></span>
                  <div className="balls">
                    {lot.superena.map(n => <b key={n}>{n}</b>)}
                    <b className="jolly" key="j">{lot.jolly}</b>
                  </div>
                </div>
              </>); })()}
              <div className="hint2" style={{ marginTop: 10, marginBottom: 0 }}>红球为 EuroJackpot 幸运号，金球为 SuperEnalotto 的 Jolly 号。卦数游戏，娱乐怡情即可。</div>
            </div>
          </div>
        )}
